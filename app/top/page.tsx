...
import { fetchAnimePage, displayTitle, formatStatus, type AnimeMedia, type PageInfo } from '@/lib/anilist'
  ...
  return (
    <div className="mt-8 flex flex-col gap-3">
      {Array.from({ length: 25 }).map((_, i) => (
        <div key={i} className="flex h-24 w-full animate-pulse items-center gap-4 rounded-xl bg-card p-3" />
      ))}
    </div>
  )
}
...
function TopContent() {
  ...
  const page = Math.min(Math.max(Number(pageParam) || 1, 1), 4)
  ...
  useEffect(() => {
    let mounted = true
    setLoading(true)
    fetchAnimePage({ sort: ['SCORE_DESC'], page, perPage: 25 })
      .then(res => {
        if (mounted) {
          setData(res)
          setLoading(false)
      ...
      {loading || !data ? (
        ...
        <>
          ...
          <div className="mt-10">
            <PaginationNav
              currentPage={page}
              lastPage={Math.min(data.pageInfo.lastPage, 4)}
              basePath="/top"
              params={{}}
            />
          </div>
