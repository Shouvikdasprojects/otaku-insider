...
export function AwardsClient({ year }: { year: number }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 space-y-14">
        {loading || !data ? (
          <div className="flex flex-col items-center justify-center space-y-6 py-20 text-muted-foreground">
            <div className="h-10 w-10 animate-spin rounded-full border-4 border-primary border-t-transparent" />
            <p>Gathering the winners for {year}...</p>
          </div>
        ) : (
          <>
            {/* ── ANIME OF THE YEAR ─────────────────────────────────────── */}
            <section aria-labelledby="aoty-heading">
              ...
              {data.overall[0] ? (
                <div className="group relative overflow-hidden rounded-3xl border border-white/[0.08] shadow-2xl">
                  <div className="absolute inset-0">
                    {data.overall[0].bannerImage ? (
                      <Image
                        src={data.overall[0].bannerImage}
                        alt=""
                        fill
                  ...
                  <Link href={`/anime/${data.overall[0].id}`} className="relative flex flex-col sm:flex-row items-start sm:items-center gap-6 p-8">
                    ...
                    >
                      <Image
                        src={data.overall[0].coverImage}
                        alt={data.overall[0].title}
                        fill
                    ...
                    <div className="flex-1 space-y-3">
                      <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
                        {data.overall[0].genres.slice(0, 3).join(' · ')}
                      </p>
                      <h3 className="text-3xl font-black leading-tight text-foreground group-hover:text-primary transition-colors sm:text-4xl">
                        {data.overall[0].title}
                      </h3>
                      {data.overall[0].englishTitle && (
                        <p className="text-sm text-muted-foreground">{data.overall[0].englishTitle}</p>
                      )}

                      <div className="flex flex-wrap items-center gap-4 pt-1">
                        {data.overall[0].averageScore && (
                          <div className="flex items-center gap-1.5">
                            <Star className="h-5 w-5 fill-primary text-primary" aria-hidden="true" />
                            <span className="text-2xl font-black text-primary">
                              {(data.overall[0].averageScore / 10).toFixed(1)}
                            </span>
                            <span className="text-sm text-muted-foreground">/10</span>
                          </div>
                        )}
                        {data.overall[0].episodes && (
                          <span className="text-sm text-muted-foreground">{data.overall[0].episodes} episodes</span>
                        )}
                        {data.overall[0].format && (
                          <span className="rounded-full border border-border bg-secondary px-3 py-1 text-xs font-medium text-muted-foreground">
                            {data.overall[0].format.replace('_', ' ')}
                          </span>
                        )}
                      </div>

                      {data.overall[0].description && (
              ...
              {data.overall.slice(1).length > 0 && (
                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  {data.overall.slice(1).map((anime: AwardAnime, i: number) => (
